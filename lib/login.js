...
const getIPs = (tokens, machines, cb) => {
    ...
    client.on("message", (topic, message) => {
        ...
        let foundIP = false;
        let ip;
        if (json.print?.net?.info) {
            // if the printer contains a networking section with it's IP and mask
            let printerIPs = json.print.net.info;

            // if there are IPs
            if (printerIPs.length != 0) {
                for (let value of printerIPs) {
                    if (value.ip !== 0) {
                        // convert ip from number to string
                        let part1 = value.ip & 0xFF;
                        let part2 = (value.ip >> 8) & 0xFF;
                        let part3 = (value.ip >> 16) & 0xFF;
                        let part4 = (value.ip >> 24) & 0xFF;
    ...
    let DONE;
    const setIPs = () => {
        logger.warn(`Machine IP doesn't appear in the messages.`);
